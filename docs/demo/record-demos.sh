...
optimize_gif() {
	...
	mv "$file" "$file.raw"
	ffmpeg -y -loglevel error -i "$file.raw" -vf "fps=7,scale=860:-1:flags=lanczos,split[s0][s1];[s0]palettegen=max_colors=64:stats_mode=diff[p];[s1][p]paletteuse=dither=bayer:bayer_scale=5" "$file"
	rm -f "$file.raw"
}
...
record() {
	...
	optimize_gif "$IMG/$name.gif"
	echo "$IMG/$name.gif ($(du -h "$IMG/$name.gif" | cut -f1))"
}

record "$IMG/demo-queue.tape"
record "$IMG/demo-watch.tape"

# Legacy name: point demo.gif at the queue clip for old links.
